...
async def add_watermark(input_path, output_path):
    """Add watermark to video file using ffmpeg"""
    ffmpeg = shutil.which('ffmpeg')
    if not ffmpeg:
        raise RuntimeError("FFmpeg not found in PATH")
    ...
    cmd = [
        ffmpeg,
        '-i', input_path,
        '-vf', "drawtext=text='ANIME ATLAS':x=20:y=20:fontsize=28:fontcolor=white:fontfile=/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf",
        '-codec:a', 'copy',
        '-y',
        '-loglevel', 'error',
        output_path
...
async def auto_rename_files(client, message):
    ...
    try:
        ...
        # Add watermark for videos only
        if media_type == "video":
            await msg.edit("**Adding watermark...**")
            watermark_path = f"watermark/{new_filename}"
            os.makedirs(os.path.dirname(watermark_path), exist_ok=True)
            try:
                await add_watermark(file_path, watermark_path)
                file_path = watermark_path
            except Exception as e:
                await msg.edit(f"Watermark processing failed: {e}")
                raise
        ...
        # Clean up files
        watermark_path = f"watermark/{new_filename}" if media_type == "video" else None
        await cleanup_files(download_path, metadata_path, thumb_path, watermark_path)
